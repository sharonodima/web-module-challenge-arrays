...
function addFlavor(array1, newFlavor){
 array1.unshift(newFlavor);
 return array1;
}
...
function removeLastFlavor(array1){
 array1.pop();
 return array1;
}
...
function getFlavorByIndex(array1, number){
...
function removeFlavorByName(array1, string){
  array1.splice(array1.indexOf(string), 1);
  return array1;
}
...
function filterByWord(array1, string){
  let filtered = [];
  for (let i = 0; i < array1.length; i++){
    if (array1[i].includes(string)){
      filtered.push(array1[i]);  
    }
  }
  return filtered;
...
function getAverageWordLength(array1){
  ...
  let average = 0;
  for(let i = 0; i < array1.length; i++){
   let words = array1[i].split(' ');
    let howLongEachStr = words.length;
    sum = sum + howLongEachStr;
...
function getRandomFlavors(array1, array2, array3, array4){
  let randomFlavors = [];
  newArray = array1.concat(array2, array3, array4);
  for (let i = 0; i < newArray.length; i++) {
    let randomFlavor = array1[Math.round(Math.random() * array1.length)];
